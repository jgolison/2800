...
# any word lists provided to this function with less than 3 words will not run
...
# Tests that the valid function works as expected 
def test_valid():
    # tests that words with 2 characters and words with more than 2 characters
    # are both handled appropriately
    ...
    assert valid(["abc", "hello", "bye"]) == "(abDS AND bcDS AND ehDS AND elDS AND llDS AND loDS AND byDS AND eyDS)"
    # tests that it alphabetizes words correctly, and handles only being given
    # one word just fine
...
def test_main_func():
    # test with more than 3 words, meaning multiple clauses will have to be created
    assert main_func(["a", "b" , "c", "d"]) == "(a-b-c-covers AND ((a->b AND b->c) OR (a->c AND c->b) OR (b->c AND c->a) OR (b->a AND a->c) OR (c->b AND b->a) OR (c->a AND a->b)) AND (aDS AND bDS AND cDS)) OR \n(a-b-d-covers AND ((a->b AND b->d) OR (a->d AND d->b) OR (b->d AND d->a) OR (b->a AND a->d) OR (d->b AND b->a) OR (d->a AND a->b)) AND (aDS AND bDS AND dDS)) OR \n(a-c-d-covers AND ((a->c AND c->d) OR (a->d AND d->c) OR (c->d AND d->a) OR (c->a AND a->d) OR (d->c AND c->a) OR (d->a AND a->c)) AND (aDS AND cDS AND dDS)) OR \n(b-c-d-covers AND ((b->c AND c->d) OR (b->d AND d->c) OR (c->d AND d->b) OR (c->b AND b->d) OR (d->c AND c->b) OR (d->b AND b->c)) AND (bDS AND cDS AND dDS)) " 
    # test with 3 words, meaning only one clause will be created
    assert main_func(["hi", "bye", "hello"]) == "(hi-bye-hello-covers AND ((hi->bye AND bye->hello) OR (hi->hello AND hello->bye) OR (bye->hello AND hello->hi) OR (bye->hi AND hi->hello) OR (hello->bye AND bye->hi) OR (hello->hi AND hi->bye)) AND (hiDS AND byDS AND eyDS AND ehDS AND elDS AND llDS AND loDS)) "
    # testing that no boolean encoding will be created if less than 3 words
    # are provided to the function
    assert main_func(["hi", "bye"]) == ""

# Tests that the covers function creates the covers list and all possible combos
def test_covers():
    # testing that when provided with more than 3 words, multiple combinations are created
    assert covers(["a", "b", "c", "d"]) == (['a-b-c-covers', 'a-b-d-covers', 'a-c-d-covers', 'b-c-d-covers'],
                                           [('a', 'b', 'c'), ('a', 'b', 'd'), ('a', 'c', 'd'), ('b', 'c', 'd')])
    # testing that when provided with less than 3 words, the lists are empty
    assert covers(["hi", "bye"]) == ([], [])
    # testing that when 3 words are given, just one combination is created
...
def test_flows():
    # testing that all possible combinations of the 6 flows are properly created
